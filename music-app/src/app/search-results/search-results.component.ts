...
import { AuthService, GetSearchVideoResponse } from '../auth.service';
...
export class SearchResultsComponent implements OnInit{
  ...
  query: string = ''
  searchResults: GetSearchVideoResponse[] = []
  ...
  searchVideos(query: string): void {
    this.authService.getSearchResults(query).subscribe(
      (data: GetSearchVideoResponse[]) => {
        this.searchResults = data
        this.searchResults.forEach((video) => {
          this.video(video.videoName)
        })
        this.searchResults.forEach((image) => {
  ...
  setDurationVideo(videoUrl: string, videoName: string): void {
    ...
    videoElement.onloadedmetadata = () => {
      ...
      const video = this.searchResults.find(vid => vid.videoName === videoName)
      if(video) {
  ...
  // Navigation to video 
  nvigationToVideo(videoId: string): void {
